...
export async function fetchPyth(ids: string[]): Promise<{ source: PythSource; prices: Record<string, PythPrice> }> {
  ...
  if (key) {
    try {
      const base = process.env.PYTH_HERMES_URL?.trim() || `${HERMES}/v2/updates/price/latest`;
  ...
  const j = await rpc("getMultipleAccounts", [accts, { encoding: "base64" }]);
  const out: Record<string, PythPrice> = {};
  (j.result?.value ?? []).forEach((v: { data: [string, string] } | null, i: number) => {
    if (!v) return;
...
export async function fetchScaled(mints: string[]): Promise<Record<string, ScaledConfig>> {
  const j = await rpc("getMultipleAccounts", [mints, { encoding: "jsonParsed" }]);
  const out: Record<string, ScaledConfig> = {};
  (j.result?.value ?? []).forEach((v: { owner: string; data: { parsed: { info: { decimals: number; supply: string; extensions: { extension: string; state: Record<string, unknown> }[] } } } } | null, i: number) => {
    if (!v) return;
...
export async function buildSnapshot(): Promise<SnapshotResponse> {
  const errors: string[] = [];
  const ids = MARKETS.flatMap((m) => [m.equityFeed, m.xFeed]);
  const [pythRes, scaled, histories] = await Promise.all([
    fetchPyth(ids).catch((e) => { errors.push(String(e)); return { source: "solana-pyth-receiver" as PythSource, prices: {} as Record<string, PythPrice> }; }),
    fetchScaled(MARKETS.map((m) => m.mint)).catch((e) => { errors.push(String(e)); return {} as Record<string, ScaledConfig>; }),
    Promise.all(MARKETS.map((m) => fetchHistory(m.x).catch(() => null))),
  ]);
  ...
  const markets: MarketSnapshot[] = MARKETS.map((m, i) => {
    ...
    const px = xp?.price ?? eq?.price ?? null;
    ...
    return {
      ...
      xPrice: xp?.price ?? null, xPublish: xp?.publish ?? null,
