    ...
    <div>
      ...
      <div className="mt-8 grid gap-4 md:grid-cols-3">
        ...
        <div className="card p-6">
          ...
          <div className="mt-6 space-y-2 text-xs">
            <div className="flex justify-between"><span className="text-dim">Prices</span><span className="num">{data?.pythSource === "hermes" ? "Pyth Hermes" : "Pyth · Solana accounts"}</span></div>
      ...
      <div className="card mt-4 overflow-hidden">
        <div className="hidden grid-cols-[1.6fr_1fr_1fr_1fr_0.9fr_1fr_1fr_1.1fr] gap-3 border-b border-line px-6 py-3 lg:grid">
          {["Asset", "Pyth equity", "Pyth xStock", "Multiplier", "Yield 12m", "Coupon bid", "Coupon ask", ""].map((h) => <div key={h} className="micro !text-[9px] text-faint">{h}</div>)}
        </div>
        {(ms.length ? ms : Array.from({ length: 6 }, () => null)).map((m, i) => (
          <motion.div key={m?.x ?? i} initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: Math.min(i * 0.035, 0.25), duration: 0.35 }} className="grid grid-cols-2 items-center gap-3 border-b border-line px-6 py-4 transition-colors last:border-0 hover:bg-surface-2/60 lg:grid-cols-[1.6fr_1fr_1fr_1fr_0.9fr_1fr_1fr_1.1fr]">
            ...
            <Cell l={`Pyth ${m?.under ?? ""}`} v={usd(m?.equity)} sub={m?.equityPublish ? ago(m.equityPublish) : undefined} />
            <Cell l="Pyth xStock" v={usd(m?.xPrice)} sub={m?.xPublish ? ago(m.xPublish) : undefined} />
      ...
      <p className="mt-4 text-xs text-faint">Coupon price per 1 d-token = xStock price × trailing 12 month multiplier growth. Bid is 8% under fair value, ask is 5% under. Multiplier history from the xStocks public API, current multiplier read from each mint on Solana mainnet.</p>
    </div>
  );
}
