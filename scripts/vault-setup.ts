...
const mkMint = async (label: string, decimals: number, authority: PublicKey, multiplier?: number, multiplierAuthority?: PublicKey) => {
...
for (const mk of markets) {
  ...
  if (k.lp && !(await conn.getAccountInfo(k.pool))) {
    ...
    await send(`init_pool ${m.symbol}`, v.initPool(admin.publicKey, k.d, k.lp, k.market, poolD, usdc));
  }
}
save();
